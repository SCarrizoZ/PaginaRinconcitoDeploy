 ...
 // this will be used to filter the products based on brands
  ...
  const applyFilter = () => {
    ...
    if (selectedCategories.length === 0 && selectedBrands.length === 0 && minPrice === 0 ) {
      ...
      console.log(selectValue === "low-price")
      console.log("ANTES: ", filteredItems)
      // change switch for if
      // if(selectValue!=='Default'){
      //   console.log("DURANTE-ANTES: ",filteredItems)
      //   // Why this below line of code is not working?
      //   switch(selectValue) {
      //     case "ascending":
      //       filteredItems = filteredItems?.sort((a,b)=>(a?.attributes?.nombre.localeCompare(b?.attributes?.nombre)))
      //       break
      //     case "descending":
      //       filteredItems = filteredItems?.sort((a,b)=>(b?.attributes?.nombre.localeCompare(a?.attributes?.nombre)))
      //       break
      //     case "high-price":
      //       filteredItems = filteredItems?.sort((a,b)=>(b?.attributes?.precio - a?.attributes?.precio))
      //       break
      //     case "low-price":
      //       filteredItems = filteredItems?.sort((a,b)=>(parseInt(a?.attributes?.precio) - parseInt(b?.attributes?.precio)))
      //       break
      //   }
      // }
      ...
      console.log("DESPUES: ", filteredItems)
  ...
  const applySort = (list) => {
    if(selectValue === 'ascending'){
      return list.sort((a,b)=>(a?.attributes?.nombre.localeCompare(b?.attributes?.nombre)))
    }
    else if(selectValue === 'descending'){
      return list.sort((a,b)=>(b?.attributes?.nombre.localeCompare(a?.attributes?.nombre)))
    }
    else if(selectValue === 'high-price'){
      return list.sort((a,b)=>(parseInt(b?.attributes?.precio) - parseInt(a?.attributes?.precio)))
    }
    else if(selectValue === 'low-price'){
      console.log(list.sort((a,b)=>(parseInt(a?.attributes?.precio) - parseInt(b?.attributes?.precio))))
      return list.sort((a,b)=>(parseInt(a?.attributes?.precio) - parseInt(b?.attributes?.precio)))
    }
    else{
      return list
    }
